        ...
        <div>
            ...
            <div className="card" style={{padding: 0, overflow: "hidden"}}>
                ...
                <div style={{
                    ...
                    <PlayerHeader
                        player={{ name: match.player1Name, elo: match.player1Elo, userId: match.player1UserId }}
                    ...
                    <PlayerHeader
                        player={{ name: match.player2Name, elo: match.player2Elo, userId: match.player2UserId }}
                ...
                {isPending && !view.canValidate && (
                    <div style={{
                        ...
                        En attente de validation par le perdant
                        ({match.winnerId === match.player1Id ? match.player2Name : match.player1Name}).
                    </div>
                )}
            </div>
...
function PlayerHeader({ player, isWinner, isMe, align }) {
  ...
  const avatar = (
      <div style={{
        ...
        <div style={{
          ...
          <Avatar userId={player.userId} name={player.name} size={72} bg={colorFromName(player.name)} />
        </div>
        {isWinner && (
            <span style={{
              position: "absolute",
              ...
              boxShadow: "0 4px 14px rgba(250,204,21,0.55), 0 0 0 2px #0B0F2A",
  ...
  // Texte collé au score : alignement opposé à `align` (avatar = bord, texte = centre)
  const textAlign = align === "left" ? "right" : "left";
  const textJustify = align === "left" ? "flex-end" : "flex-start";

  const textBlock = (
      <div style={{ textAlign, color: isWinner ? "#fff" : "#C9D1FF", minWidth: 0 }}>
        <div style={{
          fontWeight: isWinner ? 700 : 600, fontSize: 22,
        ...
        <div style={{
          ...
          fontSize: 12, color: "#8A92B2", marginTop: 6,
          display: "flex", alignItems: "center", gap: 6,
          justifyContent: textJustify,
        }}>
          <span style={{
            display: "inline-block", width: 6, height: 6, borderRadius: "50%",
            background: isWinner ? "#22C55E" : "#5A6478",
          }}/>
          Elo {player.elo}
        </div>
      </div>
  ...
  // Avatar collé au texte, le tout poussé vers le score (centre)
  return (
      <div style={{
        ...
        {align === "right" ? <>{textBlock}{avatar}</> : <>{avatar}{textBlock}</>}
      </div>
  );
}
        ...
        <div className="card" style={{
            ...
            <div style={{marginBottom: 14}}>
                ...
                <div style={{display: "flex", gap: 8}}>
                    <button type="button" onClick={() => setWinnerId(match.player1Id)}
                            style={winnerBtn(match.player1Id)}>
                        {match.player1Name}
                    </button>
                    <button type="button" onClick={() => setWinnerId(match.player2Id)}
                            style={winnerBtn(match.player2Id)}>
                        {match.player2Name}
                    </button>
                </div>
            </div>
